...
export class CharttypesComponent implements AfterViewInit {
  ...
  constructor(
    ...
    private service: ChartProviderService
  ) {}
  ...
  populateForm(): void {
    if (this.editingChartData) {
      this.xAxisInput.nativeElement.value =
        this.editingChartData.xAxisValues.join(',');
      this.yAxisInput.nativeElement.value =
        this.editingChartData.yAxisValues.join(',');
      this.apiUrlInput.nativeElement.value = this.editingChartData.apiUrl;

      this.chartTypeSelect.nativeElement.value =
        this.editingChartData.selectedChartType;
        ...
        if (selectedChartOptions) {
          ...
          this.chartOptions = {
            ...
            xaxis: {
              ...
              categories: transformedData.map((item: { x: any[] }) =>
                item.x.join(', ')
              ),
            },
            legend: {
            ...
            labels: transformedData.map((item: { x: any[] }) =>
              item.x.join(', ')
            ),
          };

          if (selectedChartOptions.chart.type == 'pie') {
            this.chartOptions.series = transformedData.map((item) =>
              parseFloat(item.y)
            );
          }
        } else {
  ...
  saveChartParams(): void {
    ...
    if (this.editingChartData != null) {
      const index = this.chartService
        .getData()
        .findIndex(
          (chart: any) => chart.chartName === this.editingChartData.chartName
        );
      if (index !== -1) {
